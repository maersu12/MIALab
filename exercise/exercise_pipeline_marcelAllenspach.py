...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(file_keys=image_keys,
                                           dir_filter=dir_filter,
                                           file_path_generator=MyFilePathGenerator(),
                                           root_dir=data_dir)  # todo: modify here
...
def load_images(image_paths):
    ...
    nameT1w, nameGT=image_paths.values()
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(fileName=nameT1w, outputPixelType=sitk.sitkFloat32),  # todo: modify here
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(fileName=nameGT, outputPixelType=sitk.sitkUInt8)  # todo: modify here
    }
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    minmax_rescaled_img = MinMaxRescaleFilter.execute(filter_, img=image_dict[structure.BrainImageTypes.T1w],params=filter_params)  # todo: modify here
